...
# # Load the raw samples from the file
# samples = np.fromfile('input.dat', dtype=np.complex64)  # Assuming the samples are stored as 32-bit complex
...
# samples_to_display = samples[0:150]  # Displaying the first 25 samples (to give 10+ periods at 2.5 samples per period)
...
# Number of samples per period = Sampling rate / Frequency = 25000 / 10000 = 2.5 samples per period
# For 5 periods, you'll need roughly 12-13 samples
samples_to_display = samples  # Displaying the first 25 samples (to give 10+ periods at 2.5 samples per period)
print(samples_to_display)
